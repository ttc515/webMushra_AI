...
MushraPage.prototype.renderCanvas = function (_parentId) {
  ...
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'Human'), xLetters, prevScalesHeight * 0.1 + YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'Most likely human'), xLetters, prevScalesHeight * 0.3 + YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'Unsure'), xLetters, prevScalesHeight * 0.5 + YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'Most likely AI'), xLetters, prevScalesHeight * 0.7 + YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'AI'), xLetters, prevScalesHeight * 0.9 + YfirstLine);
  ...
  var yOffset = prevScalesHeight + 20; // vertical space between the two sections

// Horizontal line under second slider set
canvasContext.beginPath();
canvasContext.moveTo(xDrawingStart, YfirstLine + yOffset);
canvasContext.lineTo(xDrawingEnd, YfirstLine + yOffset);
canvasContext.stroke();

// Segment lines and scale
for (var i = 0; i < scaleSegments.length; ++i) {
  canvasContext.beginPath();
  canvasContext.moveTo(xDrawingStart, prevScalesHeight * scaleSegments[i] + YfirstLine + yOffset);
  canvasContext.lineTo(xDrawingBeforeScales, prevScalesHeight * scaleSegments[i] + YfirstLine + yOffset);
  canvasContext.stroke();

  var predecessorXEnd = null;
  $('.scales').each(function(index) {
    if (index % 2 === 1) { // every 2nd slider is sound quality
      var sliderElement = $(this).prev().first();
      if (index > 1) {
        canvasContext.beginPath();
        canvasContext.moveTo(predecessorXEnd, prevScalesHeight * scaleSegments[i] + YfirstLine + yOffset);
        canvasContext.lineTo(sliderElement.offset().left + xAbsTableOffset, prevScalesHeight * scaleSegments[i] + YfirstLine + yOffset);
        canvasContext.stroke();
      }
      predecessorXEnd = sliderElement.offset().left + sliderElement.width() + xAbsTableOffset + 1;
    }
...
canvasContext.font = "1.25em Calibri";
canvasContext.textAlign = "center";
canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'excellent'), xLetters, prevScalesHeight * 0.1 + YfirstLine + yOffset);
canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'good'), xLetters, prevScalesHeight * 0.3 + YfirstLine + yOffset);
canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'fair'), xLetters, prevScalesHeight * 0.5 + YfirstLine + yOffset);
canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'poor'), xLetters, prevScalesHeight * 0.7 + YfirstLine + yOffset);
canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'bad'), xLetters, prevScalesHeight * 0.9 + YfirstLine + yOffset);
